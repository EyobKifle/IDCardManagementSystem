...
namespace Demo2
{
    public partial class StudentRequestHistoryView : Form
    {
        ...
        private int userId;
        private string studentId; // Added to store the student's ID

        public StudentRequestHistoryView(int userId, string initials, string studentId)
        {
            InitializeComponent();
            this.userId = userId;
            this.loggedInUserInitials = initials; // Use passed initials directly
            this.studentId = studentId; // Store the student's ID
        ...
        private void StudentRequestHistoryView_Load(object sender, EventArgs e)
        {
        ...
        private void LoadDataFromDatabase()
        {
            try
            {
                ...
                {
                    connection.Open();
                    using (SqlCommand command = new SqlCommand("GetSpecificStudentRequestHistory", connection))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("@StudentID", studentId); // Filter by student ID
        ...
        private void SetColumnHeaders()
        {
            ...
            if (dataGridViewRequestHistory.Columns.Contains("StudentEmail"))
            {
                dataGridViewRequestHistory.Columns["StudentEmail"].HeaderText = "Student Email";
            }

            if (dataGridViewRequestHistory.Columns.Contains("RequestDate"))
            {
                dataGridViewRequestHistory.Columns["RequestDate"].HeaderText = "Request Date";
            }
            ...
            if (dataGridViewRequestHistory.Columns.Contains("EvidencePath"))
            {
                dataGridViewRequestHistory.Columns["EvidencePath"].HeaderText = "Evidence Path";
            }
        }
        ...
        private void HideUnwantedColumns()
        {
            foreach (DataGridViewColumn column in dataGridViewRequestHistory.Columns)
            {
                ...
                if (!new[] { "RequestID", "StudentName", "StudentEmail", "RequestDate", "Status", "Reason", "EvidencePath" }.Contains(column.Name))
                {
